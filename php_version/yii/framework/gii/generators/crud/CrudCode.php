...
class CrudCode extends CCodeModel
{
	...
	public function generateActiveLabel($modelClass,$column,$htmlOptions = null)
        {
                ...
                return "\$form->labelEx(\$model,'{$column->name}', array({$hp}))";
        }
	...
	public function generateActiveField($modelClass,$column,$htmlOptions = null)
	{
		...
		if($column->type==='boolean')
			return "\$form->checkBox(\$model,'{$column->name}', array({$hp}))";
		elseif(stripos($column->dbType,'text')!==false)
			return "\$form->textArea(\$model,'{$column->name}',array('rows'=>6, 'cols'=>50, {$hp}))";
		else
		{
			if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name))
			...
			if($column->type!=='string' || $column->size===null)
				return "\$form->{$inputField}(\$model,'{$column->name}', array({$hp}))";
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "\$form->{$inputField}(\$model,'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength, {$hp}))";
			}
		}
	}
